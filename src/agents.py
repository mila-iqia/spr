...
class SPRAgent(AtariCatDqnAgent):
    ...
    def __call__(self, observation, prev_action, prev_reward, train=False, **kwargs):
        """Returns Q-values for states/observations (with grad)."""
        if train:
            model_inputs = buffer_to((observation, prev_action, prev_reward),
                device=self.device)
            return self.model(*model_inputs, train=train, **kwargs)
        else:
    ...
    def quiet_eval(self,):
        ...
        self.model.head.set_sampling(False)
        self.target_model.head.set_sampling(False)
    ...
    def sample_mode(self, itr):
        ...
        self.model.eval()
        self.target_model.eval()
        self.model.head.set_sampling(True)
        self.target_model.head.set_sampling(True)
        self.itr = itr
    ...
    def set_mode(self, mode):
        if "train" in mode:
            self.train_mode(self.itr)
        elif "sample" in mode:
            self.sample_mode(self.itr)
        elif "eval" in mode:
            self.eval_mode(self.itr)
    ...
    def target(self, observation, prev_action, prev_reward, **kwargs):
        """Returns the target Q-values for states/observations."""
        prev_action = self.distribution.to_onehot(prev_action)
        model_inputs = buffer_to((observation, prev_action, prev_reward),
                                 device=self.device)
        target_q = self.target_model(*model_inputs, **kwargs)
        return target_q
