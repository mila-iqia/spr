...
class ReplayMemory():
    def __init__(self, args, capacity):
        ...
        self.blank_latent_trans = Transition(0, torch.zeros(256, dtype=torch.float32, device=args.device), None, 0, False)

    # Adds state and action at time t, reward and terminal at time t + 1
    def append(self, state, action, reward, terminal):
        self.transitions.append(Transition(self.t, state[-1], action, reward, not terminal),
    ...
    def _get_transition(self, idx):
        ...
        for t in range(self.history - 2, -1, -1):  # e.g. 2 1 0
            if transition[t + 1].timestep == 0:
                transition[t] = self.blank_latent_trans  # If future frame has timestep 0
            else:
        ...
        for t in range(self.history, self.history + self.n):  # e.g. 4 5 6
            if transition[t - 1].nonterminal:
                ...
                transition[t] = self.blank_latent_trans  # If prev (next) frame is terminal
        return transition
