...
blank_trans = Transition(0, torch.zeros(1, 84, 84, dtype=torch.uint8), None, 0, False)
...
class ReplayMemory():
  ...
  def _get_transition(self, idx):
    ...
    for t in range(self.history - 2, -1, -1):  # e.g. 2 1 0
      if transition[t + 1].timestep == 0:
        transition[t] = blank_latent_trans  # If future frame has timestep 0
      else:
    ...
    for t in range(self.history, self.history + self.n):  # e.g. 4 5 6
      if transition[t - 1].nonterminal:
        ...
        transition[t] = blank_latent_trans  # If prev (next) frame is terminal
    return transition
  ...
  def _get_sample_from_segment(self, segment, i):
    ...
    next_state = torch.stack([trans.state for trans in transition[self.n:self.n + self.history]]).to(device=self.device)
